* inefficient code:
  global4[0..1]:i32 := global2[0..1]:i32
  global4[0..1]:i32 := 1:i32 * global4[0..1]:i32
  global3[0..1]:i32 := global0[global4..global4+1]:i32[2]
  
  global6[0..1]:i32 := global1[0..1]:i32
  global6[0..1]:i32 := 1:i32 * global6[0..1]:i32
  global0[global6..global6+1]:i32[2] := global5[0..1]:i32 + 0:i32
* TODO recs should write their result into a global so that each call site doesn't do the indexing
* TODO binary ifs
* TODO variable delays
* TODO folds can not return lambdas
* TODO interval propagation
* TODO generate out of bounds intervals
* TODO arrays which are written to by the imperative code must be marked as impure too
* TODO array constants get initialized once at the top
* TODO SOACs: actually probably better to have the "comptime" constructs like map and range translate to loops
** otherwise we must interpret the code during compilation, which in an online setting where the compiler is JS or WASM will be slower than executing WASM loops directly
** but maybe good to unroll for small arrays, but we'll still need to support SOACs in Expr
** differentiate between init time constructs (fine to not optimize that much) and sample time constructs (better to unroll?)
...
**** (Abs _ -> let b = xs in b) = (Abs _ -> xs) (but this should be handled by inline-if-used-only-once rule)
